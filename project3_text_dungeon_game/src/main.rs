...
fn main() {
    ...
    let mut events = vec![];
    let num_events = rng.gen_range(3..=5);
        ...
        match & mut events[current_event]{
            Event::Combat {                         // 몬스터와 만났을 때
                ...
                loop {
                    ...
                    if input.trim() == "공격" {
                        println!("+------------------------------------+");
                        let damage = rng.gen_range(50..=100);
            ...
            Event::Shop => { //상점에서
                ...
                loop{
                    ...
                    println !("현재 무기: {} || (판매중인 무기: 활, 가격: 50)", player.weapon);
                    println !("현재 방어구: {} || (판매중인 방어구: 갑옷 가격: 50)", player.armor);
    ...
    loop {
        ...
        if input.trim() == "공격" {
            let damage = rng.gen_range(100..=200);
            println !("플레이어가 {}의 피해를 입혔습니다!", damage);
            boss.hp -= damage;
